...
| 异常传参场景     | 未注册的 group_name 抛错；缺少参数 TypeError；不再容忍后端 RuntimeError | 已覆盖：用例直接 ERROR/FAIL |
...
注意：本测试不再容忍后端未实现错误：若在 NPU/HCCL 上调用 enable 抛出
     RuntimeError/NotImplementedError，用例将直接 ERROR/FAIL，
     由用户判断是否为框架缺口。
...
def _test_enable_default_group(rank, world_size, device_name):
    """Call enable_symm_mem_for_group with the default group name; expect None return."""
    _init_dist_hccl(rank, world_size)
    try:
        group_name = dist.group.WORLD.group_name
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", FutureWarning)
            ret = enable_symm_mem_for_group(group_name)
        assert ret is None, f"Expected None return, got {ret!r}"
        dist.barrier()
    finally:
...
def _test_enable_idempotent(rank, world_size, device_name):
    """Calling enable twice with the same group must succeed both times."""
    _init_dist_hccl(rank, world_size)
    try:
        group_name = dist.group.WORLD.group_name
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", FutureWarning)
            r1 = enable_symm_mem_for_group(group_name)
            r2 = enable_symm_mem_for_group(group_name)
        assert r1 is None and r2 is None, \
            f"Expected (None, None), got ({r1!r}, {r2!r})"
        dist.barrier()
    finally:
        if dist.is_initialized():
            dist.destroy_process_group()
...
def _test_invalid_group_name(rank, world_size, device_name):
    """An unregistered group name should raise an error."""
    _init_dist_hccl(rank, world_size)
    try:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", FutureWarning)
            raised = False
            try:
                enable_symm_mem_for_group("no_such_group_xyz")
            except (RuntimeError, ValueError, KeyError):
                raised = True
        assert raised, "Expected an error for unknown group name"
        dist.barrier()
    finally:
        if dist.is_initialized():
            dist.destroy_process_group()
